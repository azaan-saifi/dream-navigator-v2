...
export function splitAudio(inputPath: string, outputDir: string, chunkDuration: number): Promise<string[]> {
...
export async function transcribeChunks(files: CloudinaryResult[]): Promise<TranscriptionProps[]> {
    const transcribePromises = files.map(async (file) => {
        ...
        return replicate.run(`${process.env.REPLICATE_MODEL_OWNER}/${process.env.REPLICATE_MODEL_ID}`, { input }) as Promise<TranscriptionProps>;
    });
...
export function mergeTranscriptions(transcriptions: TranscriptionProps[], CHUNK_DURATION: number): MergedTranscriptionProps {
...
export async function createEmbeddings(chunks: TranscriptionChunks[]): Promise<{ embeddings: number[][], texts: string[] }> {
    try {
...
export async function storeInPinecone(embeddings: number[][], chunks: TranscriptionChunks[], texts: string[]) {
    try {
