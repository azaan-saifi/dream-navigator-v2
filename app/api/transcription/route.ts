...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (segments.length > 12) {
      console.log(
        `Warning: Audio was split into ${segments.length} chunks. Deleting extra chunks.`
      );
    ...
    const { segments: transcriptSegments } = mergeTranscriptions(
      transcriptions,
      CHUNK_DURATION
    );
    ...
    const transcription = createTranscriptionChunks(
      transcriptSegments,
      videoTitle,
      videoUrl
    );
